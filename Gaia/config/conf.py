...
def get_broker_and_backend():
    ...
    db = redis_info.get('broker', 1)
    if sentinel_args:
        broker_url = ";".join('sentinel://{}:{}/{}'.format(sentinel['host'], sentinel['port'], db) for
                              sentinel in sentinel_args)
        return broker_url
    else:
        host = redis_info.get('host')
        port = redis_info.get('port')
        backend_db = redis_info.get('backend', 2)
...
def get_proxy_db_type():
    return cf.get('proxy').get('type')
